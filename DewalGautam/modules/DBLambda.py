# Dewal Gautam DynamoDB Lambda
# Author: Dewal Gautam
# Description: Lambda to fetch latest metrics and store in DynamoDB
...
def dewal_get_latest_metric(metric_name, namespace, url):
...
def lambda_handler(event, context):
    namespace = constants.DEWAL_MONITOR_NAMESPACE
    latency_metric = constants.DEWAL_METRIC_LATENCY
    availability_metric = constants.DEWAL_METRIC_AVAILABILITY

    items = []
    for url in constants.DEWAL_URLS_TO_MONITOR:
        latency = dewal_get_latest_metric(latency_metric, namespace, url)
        availability = dewal_get_latest_metric(availability_metric, namespace, url)
    ...
    # You can now use table.put_item() to store items
